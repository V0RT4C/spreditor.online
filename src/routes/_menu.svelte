<div class={`mt-4 bg-primary w-full border border-solid border-primary rounded-lg p-2 h-16 flex lg:block justify-between items-center`}>
    ...
    {#if hasLoadedSprites}
        ...
        <div class="lg:relative lg:top-[-10px] flex justify-end items-center px-2 gap-2 float-right">
            ...
            <div class="relative w-40 text-gray-600 focus-within:text-gray-400">
                <span class="absolute inset-y-0 left-0 flex items-center pl-2">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" class="w-6 h-6"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                </span>
                <input bind:value={searchStr} on:input={() => dispatch('search')} type="search" name="q" class="py-2 w-40 text-white text-xs bg-secondary rounded-md pl-10 focus:outline-none focus:bg-white focus:text-gray-900 border border-solid border-primary" placeholder="Search id..." autocomplete="off">
            </div>
        </div>
    {/if}
</div>
